...
describe("ErrorBanner - sanitizeErrorMessage", () => {
  it("deve sanitizar erros técnicos em mensagens amigáveis", () => {
    expect(sanitizeErrorMessage("SyntaxError: Unexpected token < in JSON at position 0")).toBe(
      "Erro ao processar os dados recebidos. Tente novamente."
    );

    expect(sanitizeErrorMessage("TypeError: Failed to fetch")).toBe(
      "Não foi possível conectar ao servidor. Verifique sua internet."
    );

    expect(sanitizeErrorMessage("AbortError: signal is aborted")).toBe(
      "A conexão foi interrompida. Verifique sua internet e tente novamente."
    );

    expect(sanitizeErrorMessage("timeout: tempo excedido")).toBe(
      "O tempo de resposta excedeu o limite. Tente novamente em instantes."
    );

    expect(sanitizeErrorMessage("ENOENT: no such file or directory")).toBe(
  ...
  it("deve manter mensagens já amigáveis inalteradas", () => {
    ...
    expect(sanitizeErrorMessage("Sem dados disponíveis.")).toBe("Sem dados disponíveis.");
  });
});
